...
（2）两个运算子之中，只要有一个是字符串，则另一个会被自动转为字符串，然后执行字符串连接运算。

（3）两个运算子都为布尔值，或者一个为布尔值，另一个为数值，则布尔值转为数值（true为1，false为0），然后执行加法运算。
...
（4）运算子之中存在对象，则先调用该对象的valueOf方法，如果返回结果为原始类型的值，则运用上面三条规则；否则继续调用该对象的toString方法，再运用上面两条规则。
