...
上面的代码的原意，是要返回一个对象，但实际上返回的是`undefined`，因为JavaScript自动在return语句后面添加了分号。为了避免这一类错误，需要写成下面这样：
...
JavaScript规定，行尾的分号可以省略。事实上，确实有一些开发者行尾从来不写分号。但是，由于下面要讨论的原因，建议还是不要这个分号。

### 不使用分号的情况

分号表示一条语句的结束。有一些语法结构不需要在语句的结尾添加分号，主要是以下三种情况。
...
一般来说，在没有分号结尾的情况下，如果本行的结尾或者下一行起首的，是`(`、`[`、`+`、`-`、`/`这五个字符中的一个，分号不会被自动添加。
...
上面代码 之所以会得到“1 2 0”的结果，原因是自增和自减运算符前，自动加上了分号。上面的代码实际上等同于下面的形式。
