...
加法运算符（`+`）在JavaScript语言里面，其实包含了两个运算符，既可以处理算术的加法，也可以用作字符串连接，都写成`+`。
...
1. 如果运算子是对象，先自动转成原始类型的值（即先执行该对象的`valueOf`方法，如果结果还不是原始类型的值，再执行`toString`方法；如果是`Date`对象的实例，则先执行`toString`方法）。
2. 只要有一个运算子是字符串，则两个运算子都转为字符串，执行字符串连接运算。
...
这两种运算比较容易理解，就是逐位比较两个运算子。“或运算”的规则是，如果两个二进制位之中至少有一个位为1，则返回1，否则返回0。“与运算”的规则是，如果两个二进制位之中至少有一个位为0，则返回0，否则返回1。
