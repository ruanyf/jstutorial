...
加法运算符（+）需要注意的地方是，它除了用于数值的相加，还能用于字符串的连接。两个运算子之中只要有一个是字符串，加法运算符号就会变为字符串连接运算符，返回连接后的字符串，其他情况则是返回数值相加的和。任何类型的数据都可以使用加法运算符，会导致数据类型自动转换，关于这方面的详细讨论，参见《数据类型转换》一节。
...
关于自增和自减运算符，有一个需要注意的地方。就是它们放在变量之后，表示先返回变量操作前的值，再进行递增/递减操作；放在变量之前，表示先进行递增/递减操作，再返回变量操作后的值。
