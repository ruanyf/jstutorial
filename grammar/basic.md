...
JavaScript允许在变量赋值的同时，省略`var`声明变量。也就是说，`var a = 1`与`a = 1`，这两条语句的效果相同。但是由于这样的做法很容易不知不觉地创建全局变量（尤其是在函数内部），所以建议总是使用`var`命令声明变量。
...
在没有使用大括号格式化代码块的时候，`else`代码块总是跟随离自己最近的那个`if`语句。
