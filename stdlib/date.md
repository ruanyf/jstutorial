...
注意，如果日期采用连词线（`-`）格式分隔，且具有前导0（即ISO格式），结果会不一样。
...
上面代码中，日期字符串有没有前导0，返回的结果是不一样的。如果没有前导0，JavaScript引擎假设用户处于本地时区，所以本例返回0点0分。如果有前导0（即如果你以ISO格式表示日期），就假设用户处于格林尼治国际标准时的时区，所以返回8点0分。但是，ES6改变了这种做法，规定凡是没有指定时区的日期字符串，一律用户处于本地时区。

对于那些不需要前导0的数字（大于等于10的数字），一律视为采用ISO格式，采用格林尼治国际标准时的时区作为计时标准。
