...
JavaScript 语言具有很强的面向对象编程能力，本章介绍 JavaScript 如何进行面向对象编程。
...
构造函数作为模板，可以生成实例对象。但是，有时拿不到构造函数（比如使用别人封装的库），只能拿到实例对象，或者某个对象就根本没有构造函数，这时可以使用`Object.create()`方法，直接以某个实例对象作为模板，生成一个新的实例对象。
